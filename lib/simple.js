...
const sms = (conn, m) => {
	...
	if (m.message) {
		...
		m.body = (m.type === 'conversation') ? m.msg : (m.type === 'extendedTextMessage') ? m.msg.text : (m.type == 'imageMessage') && m.msg.caption ? m.msg.caption : (m.type == 'videoMessage') && m.msg.caption ? m.msg.caption : (m.type == 'templateButtonReplyMessage') && m.msg.selectedId ? m.msg.selectedId : ''
		m.quoted = m.msg.contextInfo != undefined ? (m.msg.contextInfo.quotedMessage != undefined ? m.msg.contextInfo.quotedMessage : false) : null
		if (m.quoted) {
