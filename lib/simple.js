...
const sms = (conn, m, store) => {
	...
	if (m.message) {
		...
		m.quoted = m.message[m.type].contextInfo != null ? m.message[m.type].contextInfo.quotedMessage : null
		if (m.quoted) {
			m.quoted.type = getContentType(m.quoted)
			m.quoted.id = m.msg.contextInfo.stanzaId
			m.quoted.isBaileys = (m.id.startsWith('3EB0') && m.id.length === 12) || (m.id.startsWith('BAE5') && m.id.length === 16)
			...
			m.getQuotedObj = async() => {
				var sL = await store.loadMessage(m.chat, m.quoted.id, conn)
				return sms(conn, sL, store)
			}
			m.quoted.delete = async() => await conn.sendMessage(m.chat, { delete: (await m.getQuotedObj().key) })
